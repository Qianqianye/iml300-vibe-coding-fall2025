const experiments = [
  ...
  },
  {
    ...
    projects: [
      ...
      { student: "Jinglin Jingan", url: "https://claude.ai/public/artifacts/96f4547f-3ec4-4bc1-a590-5d2a08d9d36f", title: "Glitch & Poor Image" },
  ...
  },
  {
    ...
    projects: [
      ...
      { student: "Claire Smith", url: "https://lovable.dev/projects/650710dd-94ac-4934-a0f-4e53b08c8d95?permissionView=main", title: "Machine Oppression" },
    ],
  },
  {
...
// One distinct bright color per experiment category
const CATEGORY_COLORS = [
  '#9acd32',  // 1: Generative Art — yellow-green
  '#00d4ff',  // 2: Interface — cyan
  '#ff6b35',  // 3: Hello Internet — orange
  '#c084fc',  // 4: Surveillance & Identity — purple
  '#f43f5e',  // 5: Glitch & Poor Image — red/rose
  '#fb923c',  // 6: Machine Oppression — amber-orange
  '#34d399',  // 7: Data Heal — emerald
  '#60a5fa',  // 8: Critical Access — blue
  '#e879f9',  // 9: Algorithmic Resistance — fuchsia
  '#facc15',  // 10: Live Coding & Performance — yellow
];

$(function () {

  /* ── Collect unique student names for filters ── */
  const allStudents = new Set();
  experiments.forEach(exp => exp.projects.forEach(p => allStudents.add(p.student)));

  /* ── Build filter buttons ── */
  const $filterBar = $('#filterBar');
  allStudents.forEach(name => {
    const firstName = name.split(' ')[0];
    $filterBar.append(`<button class="filter-btn" data-filter="${name}"><span>${firstName}</span></button>`);
  });

  /* ── Render experiments ── */
  const $list = $('#experimentsList');

  experiments.forEach((exp, i) => {
    const num = String(i + 1).padStart(2, '0');
    const color = CATEGORY_COLORS[i] || '#9acd32';

    /* Strip "Vibe Cod(ing|e) Experiment #N: " prefix for display */
    const cleanTitle = exp.title.replace(/Vibe Cod(?:ing|e) Experiment #\d+:\s*/i, '');

    const refsHtml = exp.references.map(r =>
      `<a class="ref-link" href="${r.url}" target="_blank" rel="noopener">${r.title} ↗</a>`
    ).join('');

    const projectsHtml = exp.projects.map(p =>
      `<a class="project-card" href="${p.url}" target="_blank" rel="noopener" data-student="${p.student}">
        <div class="proj-student">${p.student}</div>
    ...
    const $block = $(`
      <div class="experiment-block" data-index="${i}" style="--cat-color: ${color}">
        <div class="exp-header">
          <div class="exp-num">${num}</div>
          <div class="exp-title-wrap">
            <div class="exp-title-text" data-full="${cleanTitle}"></div>
            <div class="exp-meta">${exp.projects.length} projects · ${exp.references.length} references</div>
          </div>
        ...
        <div class="exp-body">
          <div class="exp-body-inner">
            ...
            <div class="exp-body-content">
              ...
              <div class="section-label">// Projects <span class="count-badge">${exp.projects.length} total</span></div>
  ...
  function typeText($el, text, speed, onDone) {
    ...
    const interval = setInterval(() => {
      ...
      if (i >= text.length) {
        ...
        $el.removeClass('typing').addClass('done');
        if (onDone) onDone();
      }
    }, speed);
  }
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        ...
        if ($el.hasClass('done') || $el.hasClass('typing')) return;
        const full = $el.data('full');
        const delay = parseInt($el.closest('.experiment-block').data('index')) * 60;
        setTimeout(() => typeText($el, full, 28), delay);
        observer.unobserve(entry.target);
      }
    });
  }, { threshold: 0.3 });

  $('.exp-title-text').each(function () {
    observer.observe(this);
  });

  /* ── Accordion toggle ── */
  $list.on('click', '.exp-header', function () {
    const $block = $(this).closest('.experiment-block');
    const isOpen = $block.hasClass('open');

    // close all
    $('.experiment-block.open').each(function () {
      $(this).removeClass('open').find('.exp-body').slideUp(350);
    });

    if (!isOpen) {
      $block.addClass('open').find('.exp-body').slideDown(350);
    }
  });
  ...
  $filterBar.on('click', '.filter-btn', function () {
    ...
    if (filter === 'all') {
      $('.experiment-block').removeClass('filtered-out');
      // close all open
      $('.experiment-block.open').each(function () {
        $(this).removeClass('open').find('.exp-body').hide();
      });
      return;
    }

    // Show/hide blocks based on whether student appears
    $('.experiment-block').each(function () {
      const $block = $(this);
      const hasStudent = $block.find(`.project-card[data-student="${filter}"]`).length > 0;
      if (hasStudent) {
        $block.removeClass('filtered-out');
        // auto-open the block and scroll it into view
        ...
        $block.addClass('filtered-out');
        if ($block.hasClass('open')) {
          $block.removeClass('open').find('.exp-body').slideUp(250);
        }
      }
    });

    // Highlight matching cards
    $('.project-card').css('opacity', '0.25');
    $(`.project-card[data-student="${filter}"]`).css('opacity', '1');
  });

  // Reset card opacity when filter goes back to all
  $filterBar.on('click', '.filter-btn[data-filter="all"]', function () {
    $('.project-card').css('opacity', '');
  });
